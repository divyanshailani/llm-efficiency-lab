...
6-Gate Quality & Survival Benchmark Suite for Poolside Laguna-S-2.1 NVFP4
Evaluates:
...
def test_tool_calling(base_url, headers):
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": "Search for the latest NVIDIA Blackwell B200 specifications."}
        ],
        "temperature": 0.0,
        "max_tokens": 150
    }
...
def test_json_schema(base_url, headers):
    ...
    payload = {
        "model": MODEL_NAME,
        "messages": [
            {"role": "user", "content": "Extract device info as JSON with keys 'device_name', 'vram_gb', 'quant_format': 'B200 192GB NVFP4'"}
        ],
        "temperature": 0.0,
        "max_tokens": 150
    }
...
def test_state_tracking(base_url, headers):
    ...
    payload = {
        "model": MODEL_NAME,
        "messages": [
            {"role": "user", "content": "Set current_url = 'https://poolside.ai/docs' and status = 'authenticated'."},
            {"role": "assistant", "content": "Acknowledged. State set: current_url='https://poolside.ai/docs', status='authenticated'."},
    ...
    r.raise_for_status()
    content = r.json()["choices"][0]["message"]["content"]
    passed = ("poolside.ai/docs" in content and "authenticated" in content)
    print("Result:", "PASSED" if passed else "FAILED")
...
def test_needle_recall(base_url, headers):
    ...
    needle = "SECRET CODE: LAGUNA-S21-BLACKWELL-CHAMPION-7743"
    ...
    r.raise_for_status()
    content = r.json()["choices"][0]["message"]["content"]
    passed = ("LAGUNA-S21-BLACKWELL-CHAMPION-7743" in content)
    print("Result:", "PASSED" if passed else "FAILED")
...
def run_survival_benchmark(base_url, api_key, output_json):
    ...
    print("=" * 70)
    print("LAGUNA-S-2.1 NVFP4 6-GATE SURVIVAL BENCHMARK SUITE")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Laguna-S-2.1 NVFP4 Survival Benchmark")
    ...
    parser.add_argument("--output", type=str, default="devices_and_hardware/b200_laguna_s_21_nvfp4/results/survival_results.json", help="Path to save output JSON")
