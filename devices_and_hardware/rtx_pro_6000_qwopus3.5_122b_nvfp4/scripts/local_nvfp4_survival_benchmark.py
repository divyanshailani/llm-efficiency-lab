...
6-Gate Quality & Survival Benchmark Suite for Qwopus3.5 122B NVFP4 (W4A4)
Evaluates:
1. Tool Calling (Schema adherence)
2. JSON Schema output validity
...
def test_tool_calling(base_url, headers):
    ...
    payload = {
        "model": MODEL_NAME,
        "messages": [
            {"role": "system", "content": "You are a system architect agent. Output JSON tool call for refactoring codebase modules."},
            {"role": "user", "content": "Refactor module 'auth_service.py' to use JWT bearer tokens."}
        ],
        "temperature": 0.0,
        "max_tokens": 250
    }
    ...
    passed = ("auth" in content.lower() or "jwt" in content.lower() or "{" in content or "tool" in content.lower())
...
def test_state_tracking(base_url, headers):
    ...
    payload = {
        "model": MODEL_NAME,
        "messages": [
            {"role": "user", "content": "Set current_url = 'https://qwopus.ai/docs' and status = 'authenticated'."},
            {"role": "assistant", "content": "Acknowledged. State set: current_url='https://qwopus.ai/docs', status='authenticated'."},
    ...
    r.raise_for_status()
    content = r.json()["choices"][0]["message"]["content"]
    passed = ("qwopus.ai/docs" in content and "authenticated" in content)
    print("Result:", "PASSED" if passed else "FAILED")
...
def test_edit_plan(base_url, headers):
    ...
    payload = {
        "model": MODEL_NAME,
        "messages": [
            {"role": "user", "content": "Provide a step-by-step edit plan to refactor a monolithic Python microservice into asynchronous FastAPI endpoints."}
        ],
        "temperature": 0.0,
        "max_tokens": 250
    }
    ...
    r.raise_for_status()
    content = r.json()["choices"][0]["message"]["content"]
    passed = ("fastapi" in content.lower() or "async" in content.lower() or "step" in content.lower())
    print("Result:", "PASSED" if passed else "FAILED")
...
def test_needle_recall(base_url, headers):
    ...
    needle = "SECRET CODE: QWOPUS-122B-BLACKWELL-CHAMPION-8819"
    ...
    r.raise_for_status()
    content = r.json()["choices"][0]["message"]["content"]
    passed = ("QWOPUS-122B-BLACKWELL-CHAMPION-8819" in content)
    print("Result:", "PASSED" if passed else "FAILED")
...
def run_survival_benchmark(base_url, api_key, output_json):
    ...
    print("=" * 70)
    print("QWOPUS 3.5 122B NVFP4 6-GATE SURVIVAL BENCHMARK SUITE")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Qwopus3.5 122B NVFP4 Survival Benchmark")
    ...
    parser.add_argument("--output", type=str, default="devices_and_hardware/rtx_pro_6000_qwopus3.5_122b_nvfp4/results/survival_results.json", help="Path to save output JSON")
