...
MODEL = "deckard-40b-q8"

def test_model(messages, tools=None, max_tokens=1024):
    payload = {
        "model": MODEL,
        "messages": messages,
        "temperature": 0.0,
...
def run_quality_gates():
    print("=== Starting Strict Agent Survival Benchmark (Deckard 40B Q8) ===\n")
    results = {}
    passed = 0
    total = 0
    ...
    debug_pass = False
    if code:
        print("    [Warning] Executing generated code in a local subprocess (isolated but NOT fully sandboxed)...")
        try:
            with open("temp_debug_test.py", "w") as f:
                f.write(code + "\n\nprint(add(2, 3))")
            out = subprocess.check_output(["python3", "temp_debug_test.py"], timeout=5, stderr=subprocess.STDOUT)
            if out.decode('utf-8').strip() == "5":
                debug_pass = True
        except Exception as e:
            print(f"RAW MSG: {msg}")
    ...
    if code:
        print("    [Warning] Executing generated code in a local subprocess (isolated but NOT fully sandboxed)...")
        try:
            with open("temp_plan_test.py", "w") as f:
                f.write(code + "\n\ntry:\n    hello()\n    world()\nexcept:\n    pass")
            out = subprocess.check_output(["python3", "temp_plan_test.py"], timeout=5, stderr=subprocess.STDOUT)
            out_str = out.decode('utf-8').lower()
            if "apple" in out_str and "banana" in out_str:
                plan_pass = True
        except:
            pass
    ...
    content = msg.get("content", "") if msg else ""
    reasoning = msg.get("reasoning_content", "") if msg else ""
    
    content = remove_think_tags(content).strip()
    reasoning = remove_think_tags(reasoning).strip()
    ...
    if not final_answer and reasoning:
        ...
        answer_source = "reasoning_content (fallback)"
        print("    [Warning] Using reasoning_content as fallback because main content was empty.")
        
    if "DELTA-9" in final_answer.upper():
        print(f"  Long Context 4K: ✅ PASS (Source: {answer_source})")
        passed += 1
    else:
